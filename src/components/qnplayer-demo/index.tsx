...
function StreamContainer() {
  const [url, setUrl] = React.useState(
    "http://pili-hls.qnsdk.com/sdk-live/timestamp.m3u8"
  );
  ...
  const [mediaContainer, setMediaContainer] =
    React.useState<MeidaContainerType>("media-container-1");
  ...
  React.useEffect(() => {
    ...
    player
      .play(url, document.getElementById(mediaContainer) as HTMLElement)
      .catch((e: any) => {
  ...
  return (
    <div id="stream-container">
      <div id="stream-url-container">
        <label htmlFor="stream-url-input" id="stream-url-label">
          地址1：
        </label>
        <input
          type="text"
      ...
      <div id="control-container">
        <div id="config-container">
          ...
          <div className="config-item">
            ...
            <select
              ...
              value={volumn}
              onChange={(e) =>
                setVolumn(parseFloat(e.target.value) as VolumnType)
              }
            >
          ...
          <div className="config-item">
            ...
            <select
              ...
              onChange={(e) =>
                setMediaContainer(e.target.value as MeidaContainerType)
              }
            >
        ...
        <div id="stream-control-container">
          ...
          <button className="stream-btn" onClick={() => player.release()}>
            release
          </button>
          <button className="stream-btn" onClick={() => player2.release()}>
            release
          </button>
          <button
            className="stream-btn"
            onClick={async () => {
              try {
                await player.play(
                  url,
                  document.getElementById('media-container-1') as HTMLElement
                );
              } catch (e) {
          ...
          <button
            ...
            onClick={async () => {
              try {
                await player2.play(
                  url2,
                  document.getElementById('media-container-2') as HTMLElement
                );
              } catch (e) {
                // 处理播放失败的情况，弹出 UI，用户点击后播放
                // 这里是利用浏览器 controls 来实现，也可以自己创建 UI，用户点击后执行 player.resume() 方法
                console.log("play fail", e);
                setControls(true);
                player2.setConfig({ controls: true });
              }
            }}
          >
            play
          </button>
          <button className="stream-btn" onClick={() => player.stop()}>
            stop
          </button>
          <button className="stream-btn" onClick={() => player2.stop()}>
            stop
          </button>
          <button className="stream-btn" onClick={() => player.pause()}>
            pause
          </button>
          <button className="stream-btn" onClick={() => player2.pause()}>
            pause
          </button>
          <button className="stream-btn" onClick={() => player.resume()}>
            resume
          </button>
          <button className="stream-btn" onClick={() => player2.resume()}>
            resume
          </button>
        </div>
      </div>
      <div className="stream-content-container">
        <div
          data-v="container-1"
          id="media-container-1"
          className="media-container"
        ></div>
        <div
          data-v="container-2"
          id="media-container-2"
          className="media-container"
        ></div>
      </div>
    </div>
...
function LogContainer({ logs }: { logs: any[] }) {
  ...
  const logState = logs.some((item) => item.level === "error")
    ? "not-good"
    : "good";
  return (
    <>
      <div className={`log-container-control ${logState}`} onClick={toggleLog}>
        LOG
      </div>
      <div id="log-container" style={{ top: top }}>
        <div className="log-item">{`SDK version: ${player.version}`}</div>
        <div className="log-item">{`browser report: ${JSON.stringify(
          player.getBrowserReport()
        )}`}</div>
        <div className="log-item">{`player support: ${JSON.stringify(
          player.getPlayerSupport()
        )}`}</div>
        {logs.map((logItem) => (
...
function LogItem({ logItem }: { logItem: any }) {
  return (
    <div className="log-item">{`${logItem.time} [${logItem.level}] ${logItem.msg}`}</div>
  );
}

function QNPlayerDemo() {
  ...
  useEffect(() => {
    ...
    player.on("error", (data: any) => {
      alert(
        `error: code: ${data.code}, errorType: ${data.errorType}, msg: ${data.msg}`
      );
    });
